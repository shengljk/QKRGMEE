...
expansionCoefficient(1) = stepSizeFeatureVector * aprioriErr(1) * ((alpha/(paramKernel.^2))*exp(-aprioriErr(1).^2/2/paramKernel.^2)+((1-alpha)/(paramKernel.^2))*exp(-aprioriErr(1).^2/2/paramKerne2.^2));
% start
for n = 2:trainSize
    % training
    % filtering
