function [weightVector,biasTerm,learningCurve]= ...
    APA1s(K,trainInput,trainTarget,stepSizeWeightVector,stepSizeBias,flagLearningCurve)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Function LMS1:
...
if flagLearningCurve
    ...
    learningCurve(1:K) = mean(trainTarget.^2)*ones(K,1);
else
    learningCurve = [];
end
...
for n = 1:trainSize-K+1
    ...
    if flagLearningCurve
        learningCurve(n+K-1) = mean(aprioriErr.^2);
    end
end
return
